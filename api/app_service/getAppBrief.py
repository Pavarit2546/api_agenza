...
def get_app_brief_service(creds, version, host, workspace_id, app_id):
    method = 'POST'
    action = 'GetAppBrief'
    service = 'app'
    url_path = '/'

    # 1. เตรียม Body Data ตามเอกสาร (เรียงลำดับให้เป๊ะเพื่อ Signature)
    data = OrderedDict([
    ...
    try:
        ...
        # ป้องกันกรณี Server ตัดการเชื่อมต่อ (Empty Response)
        if not resp.text.strip():
            return {
                "error": True,
                "status_code": resp.status_code,
                "error_message": f"Gateway rejected request (Empty Response: {resp.status_code})"
            }
        ...
        return {
            "error": False,
            "data": response_json.get("Result", {}).get("AppBriefInfo")
        }
